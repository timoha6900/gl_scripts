...
Where <arg>:
	...
	-listl	list of packages that can be start
	--help  call help\n"
...
function get_act_name {
	activity_name=$(adb shell cmd package resolve-activity -c $intent $pkg | sed -n '/name=/s/^.*name=//p')
}
...
adb shell am start $pkg/$activity_name > /dev/null
